...
class LoadParameters(object):
    ...
    def reload_checkpoint(self, path=None, root=None, requires_grad=True):
        ...
        """
        if path is None: path = "checkpoint.pth"
        if root is None: root = self.params.dump_path
...
class Trainer(object):
    ...
    def get_generation_statistics(self, task):
        ...
        for info_type, infos in self.infos_statistics.items():
            ...
            aggregated_infos = torch.cat([F.pad(info, (0, max_val - info.shape[0])).unsqueeze(-1) for info in infos], -1).sum(-1)
            non_zeros = aggregated_infos.nonzero(as_tuple=True)[0]
            vals = [(non_zero.item(), "{:.2e}".format((aggregated_infos[non_zero] / aggregated_infos.sum()).item())) for non_zero in non_zeros]
    ...
    def reload_checkpoint(self, path=None, root=None, requires_grad=True):
        ...
        assert os.path.isfile(checkpoint_path), "Checkpoint path does not exist, {}".format(checkpoint_path)
    ...
    def enc_dec_step(self, task):
        ...
        if self.params.use_skeleton:
            x2, len2 = self.env.batch_equations(self.env.word_to_idx(
